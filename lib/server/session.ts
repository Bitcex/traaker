...
const getSessionPassword = () => {
  const value = process.env.POLYMARKET_SESSION_SECRET;
  if (!value || value.length < 32) {
    throw new Error("Missing or weak POLYMARKET_SESSION_SECRET.");
  }
  return value;
};

const buildSessionOptions = (): SessionOptions => ({
  cookieName: "traak_polymarket_session",
...
export const getSession = async () =>
  getIronSession<PolymarketSessionData>(await cookies(), buildSessionOptions());
