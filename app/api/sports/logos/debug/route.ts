    ...
    return NextResponse.json(
      {
        ...
        finalResults: resolved.map((item) => ({
          ...
          participantType: item.result.participantType ?? null,
          matchedParticipant: item.result.teamDisplayName ?? item.result.teamName ?? null,
          matchedSource: item.result.providerUsed ?? null,
          rawLogoUrl: item.matchedPolymarketTeamLogoUrl ?? item.result.logoUrl ?? null,
          ...
          lookupMs: item.result.lookupMs ?? null,
          cacheHit: item.result.cacheHit ?? null,
          ...item.result,
        })),
      },
